...
RUN uv run --with huggingface-hub hf download Supertone/supertonic-2 --local-dir supertonic-data
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
  CMD curl -f http://localhost:10209/info || exit 1
...
# Default command
CMD ["python", "-m", "wyoming_supertonic", "--data-dir", "/app/supertonic-data", "--uri", "tcp://0.0.0.0:10209"]
